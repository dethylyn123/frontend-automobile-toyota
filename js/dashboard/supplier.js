...
async function getData(url = "", keyword = "") {
  ...
  const response = await fetch(
    backendURL + "/api/supplier" + queryParams,
    {
      headers: {
...
const form_suppliers = document.getElementById("form_suppliers");

form_suppliers.onsubmit = async (e) => {
  console.log('Form submitted'); // Add this line
  e.preventDefault();

  // Disable button
  console.log('Disabling button'); // Add this line
  document.querySelector("#form_suppliers button[type = 'submit']").disabled = true;
  document.querySelector("#form_suppliers button[type = 'submit']").innerHTML = 
  `<div class="col-sm-12 d-flex justify-content-center align-items-center">
      <div class="spinner-border" role="status">
          <span class="visually-hidden">Loading...</span>
      </div>
  ...
  const formData = new FormData(form_suppliers);
  ...
  // const id = document.querySelector('#form_suppliers input[type="hidden"]').value;
  // const forUpdate = id.length > 0 ? true : false;

  //   fetch API property owner store endpoint
  response = await fetch(
    backendURL + "/api/supplier",
    {
      method: "POST",
      headers: {
        Accept: "application/json",
  ...
  // For Update
    ...
    response = await fetch(
      backendURL + "/api/supplier/" + for_update_id,
      {
  ...
  if (response.ok) {
    ...
    // Reset Form
    form_suppliers.reset();

    // // Refresh the page
    // location.reload(); 

    successNotification("Successfully" + (for_update_id == "" ? " created":" updated") + " supplier.", 10);
  ...
  document.querySelector("#form_suppliers button[type='submit']").disabled = false;
  document.querySelector("#form_suppliers button[type='submit']").innerHTML = "Submit";
};
...
if (response.ok) {
    ...
    document.getElementById("supplier_name").value = json.supplier_name;
    document.getElementById("part").value = json.part;
    ...
    document.querySelector("#form_suppliers button[type='submit']").innerHTML = "Update Info";
