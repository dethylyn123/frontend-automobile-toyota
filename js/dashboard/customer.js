...
async function getData(url = "", keyword = "") {
  ...
  const response = await fetch(
    backendURL + "/api/customer" + queryParams,
    {
      headers: {
  ...
  if (response.ok) {
    ...
    json.data.forEach((element) => {
      ...
      container += `<div class="col-sm-12">
                    <div class="card w-100 mt-3" data-id="${element.customer_id}">
                    ...
                    <div class="row">
                        ...
                        <div class="card-body">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a class="dropdown-item" href="#" id="btn_edit" data-id="${element.customer_id}">Edit</a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="#" id="btn_delete" data-id="${element.customer_id}">Delete</a>
                                    </li>
                                </ul>
                            </div>

                            <div>
                            <h6 class="card-title"><b>Customer:</b>     ${element.customer_name}</h5>
                            <h6 class="card-text"><b>Gender:</b>     ${element.gender}</h6>
                            <h6 class="card-title"><b>Address:</b>     ${element.address}</h5>
                            <h6 class="card-text"><b>Phone:</b>     ${element.phone}</h6>
                            <h6 class="card-title"><b>Annual Income:</b>     ${element.annual_income}</h5>
...
const form_customers = document.getElementById("form_customers");

form_customers.onsubmit = async (e) => {
  console.log('Form submitted'); // Add this line
  e.preventDefault();

  // Disable button
  console.log('Disabling button'); // Add this line
  document.querySelector("#form_customers button[type = 'submit']").disabled = true;
  document.querySelector("#form_customers button[type = 'submit']").innerHTML = 
  `<div class="col-sm-12 d-flex justify-content-center align-items-center">
      <div class="spinner-border" role="status">
          <span class="visually-hidden">Loading...</span>
      </div>
  ...
  const formData = new FormData(form_customers);
  ...
  // const id = document.querySelector('#form_customers input[type="hidden"]').value;
  ...
  // For Update
    ...
    response = await fetch(
      backendURL + "/api/customer/" + for_update_id,
      {
  ...
  if (response.ok) {
    ...
    // Reset Form
    form_customers.reset();

    // // Refresh the page
    // location.reload(); 

    successNotification("Successfully" + (for_update_id == "" ? " created":" updated") + " customer.", 10);
  ...
  document.querySelector("#form_customers button[type='submit']").disabled = false;
  document.querySelector("#form_customers button[type='submit']").innerHTML = "Submit";
};
...
const deleteAction = async (e) => {
  ...
  confirmDeleteBtn.addEventListener('click', async () => {
    ...
    if (response.ok) {
      ...
      successNotification("Successfully deleted customer", 10);
...
if (response.ok) {
    ...
    document.getElementById("customer_name").value = json.customer_name;
    document.getElementById("gender").value = json.gender;
    document.getElementById("address").value = json.address;
    document.getElementById("phone").value = json.phone;
    document.getElementById("annual_income").value = json.annual_income;
    ...
    document.querySelector("#form_customers button[type='submit']").innerHTML = "Update Info";
